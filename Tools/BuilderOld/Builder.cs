...
public static class Builder
{
    ...
    public const string AssemblerPath       = "../nasm.exe";
    public const string CompilerPath        = "../GCC/bin/i686-elf-gcc.exe";
    public const string LinkerPath          = "../GCC/bin/i686-elf-ld.exe";
    public const string LiminePath          = "../Limine/";
    public const string LimineExePath       = "../Limine/limine-deploy.exe";
    public const string ObjsPath            = "../../Bin/Objs/";
    public const string OutputPath          = "../../Images/";
    public const string KernelPath          = "../../Kernel/";
    public const string LinkerScriptPath    = "../../Kernel/Linker.ld";
    ...
    public static bool AssembleKernel()
    {
        ...
        foreach (string file in Directory.EnumerateFiles(Path + KernelPath + "src/", "*.asm", SearchOption.AllDirectories))
        {
    ...
    public static bool CompileKernel()
    {
        List<string> files_in = new List<string>();
        List<string> files_out = new List<string>();
        foreach (string file in Directory.EnumerateFiles(Path + KernelPath + "src/", "*.cpp", SearchOption.AllDirectories))
        {
            string fname = file.Replace("\\", "/");
            fname = fname.Substring(fname.LastIndexOf('/') + 1);
            files_in.Add(file);
    ...
    public static bool CreateISO()
    {
        ...
        iso.Build(Path + OutputPath + "AerOS.iso");
    ...
    public static bool StartVM()
    {
        try
        {
            Thread thread = new Thread(() => PipeMain("AerOS"));
            thread.Start();
            Process proc = Process.Start(VMWarePath, Path + "../VMware/vmware_mach.vmx");
            proc.WaitForExit();
