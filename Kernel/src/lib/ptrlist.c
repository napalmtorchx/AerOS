...
{
    if (list == NULL) { debug_error("ptrlist_free(%p, %d) - Attempt to free null pointer list", list, free_entries); return; }
...
{
    if (list == NULL) { debug_error("ptrlist_remove(00000000, %d, %d) - Attempt to erase entry from null pointer list", index, free_ptr); return false; }
...
{
    if (list == NULL) { debug_error("ptrlist_get(%p, %d) - Null list pointer", list, index); return NULL; }
    if (index < 0 || index >= (int)list->count) { debug_error("ptrlist_get(%p, %d) - Index out of bounds", list, index); return NULL; }
