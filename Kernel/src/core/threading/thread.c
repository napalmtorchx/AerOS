...
{
    if (thread == NULL) { debug_error("thread_free(%p) - Null thread pointer", thread); return; }
...
{
    if (thread == NULL) { debug_error("thread_clock(%p) - Null thread pointer", thread); return; }
    thread->context.time.ticks++;
}

void thread_print(thread_t* thread)
{
    debug_log("%s UID:%p Stack:%p-%p Entry:%p Args:%d Name:%s\n", 
            DEBUG_THREAD, 
            thread->uid,
