...
void taskmgr_init(void)
{
    ...
    debug_log("%s Initialized task manager\n", DEBUG_OK);
}
...
void taskmgr_init_kthread(void)
{
    ...
    kthread->context.stacksz  = kernel_stack_size();
    kthread->context.stack    = kernel_stack_start();
...
void taskmgr_schedule(bool unlock)
{
    ...
    while (true)
    {
        ...
        if (thread == NULL) { debug_error("taskmgr_schedule(%d) - Null thread located in scheduler", unlock); return; }
...
{
    if (thread == NULL) { debug_error("taskmgr_load(%p) - Null thread pointer", thread); return; }
    ptrlist_add(&_threads, thread);
    debug_log("%s Loaded thread - UID:%p Name:%s\n", DEBUG_INFO, thread->uid, thread->name);
}

void taskmgr_unload_index(int index)
{
    _lock = true;
    if (index < 0 || index >= _threads.count) { debug_error("taskmgr_unload_index(%d) - Index out of bounds", index); _lock = false; return; }

    thread_t* thread = (thread_t*)ptrlist_get(&_threads, index);
    if (thread->uid == KERNEL_UID) { debug_log("%s Attempt to unload kernel thread was detected\n", DEBUG_WARN); _lock = false; return; }
    ...
    debug_log("%s Unloaded thread - UID:%p Name:%s\n", DEBUG_INFO, uid, _temp);
    _lock = false;
}
...
{
    _lock = true;
    if (thread == NULL) { debug_error("taskmgr_unload(%p) - Null thread pointer", thread); _lock = false; return; }
    ...
    debug_error("taskmgr_unload(%p) - Failed to unload thread", thread);
    _lock = false;
}
...
{
    _lock = true;
    if (thread == NULL) { debug_error("taskmgr_start(%p) - Null thread pointer", thread); _lock = false; return; }
    thread->context.state = THREAD_RUNNING;
    debug_log("%s Started thread - UID:%p Name:%s\n", DEBUG_INFO, thread->uid, thread->name);
    _lock = false;
}
...
{
    _lock = true;
    if (thread == NULL) { debug_error("taskmgr_kill(%p) - Null thread pointer", thread); _lock = false; return; }
    thread->context.state = THREAD_TERMINATED;
    debug_log("%s Terminated thread - UID:%p Name:%s\n", DEBUG_INFO, thread->uid, thread->name);
    _lock = false;
}
