...
void idt_init_exceptions()
{
    ...
    debug_log("%s Initialized exception descriptors\n", DEBUG_INFO);
}
...
void idt_init_irqs()
{
    ...
    debug_log("%s Initialized irq descriptors\n", DEBUG_INFO);
}
...
void idt_exception_handler(irq_context_t* context)
{
    ...
    debug_log("%s System Exception %s%s\n", DEBUG_ERROR, (context->irq >= n ? "" : "- "), (context->irq >= n ? "" : EXCEPTION_MSGS[context->irq]));
    debug_error("Fatal system exception");
}
...
void idt_irq_handler(irq_context_t* context)
{
    ...
    {
        if (context->irq != IRQ0) { debug_log("%s Unhandled interrupt %d\n", DEBUG_WARN, context->irq - 32); }
        irq_ack(context);
    }
}
...
{
    if (protocol == NULL) { debug_error("irq_register(%2x, 00000000) - Attempt to register interrupt with null protocol"); return; }
    _protocols[irq] = protocol;
    debug_log("%s Registered IRQ protocol - IRQ:%2x Protocol:%p\n", DEBUG_INFO, irq, protocol);
}
...
void irq_unregister(IRQ irq)
{
    ...
    _protocols[irq] = NULL;
    debug_log("%s Unregistered IRQ protocol - IRQ:%2x Protocol:%p\n", DEBUG_INFO, irq, protocol);
}

void irq_ack(irq_context_t* context)
{
    if (context->irq >= 40) { outb(0xA0, 0x20); }
    outb(0x20, 0x20);
}
