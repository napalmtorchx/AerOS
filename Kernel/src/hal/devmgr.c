...
{
    if (dev == NULL) { return 0; } //debug_exception("devmgr_register(%p) - Null device pointer"); return 0; }
...
{
    if (!devmgr_validate(dev, true)) { return; } //debug_exception("devmgr_unregister(%p) - Invalid device uid"); return; }
...
bool devmgr_start(duid_t dev, void* arg)
{
    ...
    if (device == NULL) { return false; } //debug_exception("devmgr_start(%p, %p) - Failed to start device", dev, arg); return false; }
...
{
    device_t* device = devmgr_from_uid(dev);
    if (device == NULL) { return false; } //debug_exception("devmgr_stop(%p) - Failed to stop device", dev); return false; }
...
device_t* devmgr_from_name(const char* name)
{
    ...
    if (dev == NULL) { return NULL; } //debug_exception("devmgr_from_name(%s) - No device found with specified name", name); return NULL; }
    return dev;
}
